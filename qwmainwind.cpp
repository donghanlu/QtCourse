...
{ //信号与槽的关联，当函数带有参数时，必须写明参数的类型
    // Qt6 中 QFontComboBox 已无 currentIndexChanged(const QString&) 信号，改用 currentTextChanged
    connect(spinFontSize, &QSpinBox::valueChanged,
            this, &QWMainWind::on_spinBoxFontSize_valueChanged);

    connect(comboFont, &QFontComboBox::currentTextChanged,
            this, &QWMainWind::on_comboFont_currentIndexChanged);
}
